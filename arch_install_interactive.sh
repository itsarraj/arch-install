...
main() {
    ...
    local skip_prompts=false  # Flag to control interactive prompts

    for i in "${!steps[@]}"; do
        echo -e "\n=== Step $((i+1))/${#steps[@]}: ${steps[$i]} ==="

        # Skip confirmation if user chose to continue without prompts
        if [ "$skip_prompts" = true ]; then
            echo "Auto-proceeding (continue without prompts)..."
            ${actions[$i]}
            continue
        fi
        ...
        # Interactive prompt
        while true; do
            read -p "Proceed? ([Y]es/[S]kip/[A]bort/[C]ontinue without prompts): " choice
            case "${choice,,}" in
                y|yes|"")
                    echo "Running step..."
                    ${actions[$i]}
                ...
                a|abort)
                    ...
                    ;;
                c|continue)
                    echo "Continuing without further prompts..."
                    skip_prompts=true
                    ${actions[$i]}  # Execute current step
                    break
                    ;;
                *)
                    echo "Invalid choice. Please enter Y/y, S/s, A/a, or C/c."
                    ;;
            esac
        done
    done
